...
<div class="jbr-wrap text-left border rounded text-left">
    <div class="cats-box mlb-res rounded bg-white d-flex align-items-center justify-content-between px-3 py-3">
        <div class="cats-box rounded bg-white d-flex align-items-start" style="flex: 1">
            <div class="text-center lawyer-list-profile tw-mr-2">
                @if(!$lawyer->is_premium || !$lawyer->user->profile_image)
                    <img style="width: 130px !important; max-width: 160px; min-width: 100px" class="!tw-h-[150px]" src="{{ asset('images\dashboard\users\profile_default.jpg') }}" alt="{{ $lawyer->full_name_fr }}">
                @else
                    <img style="width: 130px !important; max-width: 160px; min-width: 100px" class="!tw-h-[150px]" src="{{ asset($lawyer->user->profile_image) }}" alt="{{ $lawyer->full_name_fr }}">
                @endif
            </div>
            <div class="cats-box-caption px-2  lawyer-card-content w-100">
                ...
                <div class="d-block mb-2 position-relative">
                    @if (!!$lawyer->address)
                        <p class="m-0 {{ !$lawyer->is_premium ? 'lawyer-address' : '' }}"><span class="text-muted medium">
