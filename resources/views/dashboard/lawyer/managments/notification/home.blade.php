<x-dashboard.lawyer.master title="Messanger">
    <script src="{{ asset('assets/notification/js/jquery_v3.4.1/dist/jquery.min.js') }}"></script>
    <script src="{{ asset('assets/notification/js/home.js') }}"></script>
    <script src="{{ asset('assets/notification/js/app.js') }}"></script>
    <link rel="stylesheet" href="{{ asset('assets/notification/css/home.css') }}">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="list-group">
                            ...
                            @php
                                $client = App\Models\Client::where('id', $row->client_id)->first();

                                $auth = App\Models\UsersPeople::where('user_id', auth()->user()->id)->get();
                                // print_r($client);
                            @endphp
                            <a href="javascript:;" onclick="openChatBox({{ $client }}, {{ $row }} ,{{$auth}});" class="list-group-item list-group-action-item">
                                <div class="d-flex" style="border:0px solid red">
                                    <span id="{{ !!$client->client_name ? $client->client_name : "" }}"> {{ !!$client->client_name ? $client->client_name : ""  }} </span>
